...
public class UIelementsXml_config extends CommonBasePage {
	...
	public WebElement getObjectFromXml(String locatorKeyValue)
			throws InterruptedException {
		final String SPLITTER = "@#@";
		final String SPLITTER_MULTIPLE_Values = "||";
		WebElement element = null;
		String locator = ReadXMLConfigNodes_UIelement
				.GetLocatorFromUIelementsXml(locatorKeyValue);
		// System.out.println("locator is :" + locator);

		// implement fall back
		String[] multiplelocatorElement = locator.split(Pattern
				.quote(SPLITTER_MULTIPLE_Values));
		for (int i = 0; i < multiplelocatorElement.length; i++) {
			if (element == null) {

				String[] locatorElement = multiplelocatorElement[i]
						.split(SPLITTER);
				System.out.println("----->>" + locatorElement[1]);
				System.out
						.println("#***************@ Locating elements @*********************#");
				...
				try {
					...
					case "id":
						...
						break;
					case "className":
						waitForPageLoad(10, By.className(locatorElement[1]),
								driver);
						element = driver.findElement(By
								.className(locatorElement[1]));
						break;
					case "name":
						waitForPageLoad(10, By.name(locatorElement[1]), driver);
						element = driver
								.findElement(By.name(locatorElement[1]));
						break;
					case "linkText":
						waitForPageLoad(10, By.linkText(locatorElement[1]),
								driver);
						element = driver.findElement(By
								.linkText(locatorElement[1]));
						break;
					case "cssSelector":
						waitForPageLoad(10, By.cssSelector(locatorElement[1]),
								driver);
						element = driver.findElement(By
								.cssSelector(locatorElement[1]));
						break;
					default:
						System.out
								.println("**$$$$** not a valid locator ***$$$***");
					}
				} catch (Exception e) {
	...
	public List<WebElement> getObjectFromXml1(String locatorKeyValue)
			throws InterruptedException {
		final String SPLITTER = "@#@";
		...
		try {
			for (int i = 0; i <= 10; i++) {
				...
				case "id":
					element = driver
							.findElements(By.id(locator.split(SPLITTER)[1]));
					break;
				case "className":
				...
				case "name":
					...
					break;
				case "linkText":
					waitForPageLoad(10, By.xpath(locator.split(SPLITTER)[1]),
							driver);
	...
	private void waitForPageLoad(long seconds, final By locator,
			WebDriver driver) {
